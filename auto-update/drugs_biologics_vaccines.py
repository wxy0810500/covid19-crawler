...
ax.set_ylabel('Number of Clinical Trials', fontdict={'fontsize': 14})
ax.set_title('Number of clinical trials on COVID-19 in each country', fontdict={'fontsize': 20})
ax.set_xticks(np.arange(len(df_cos)))
ax.set_xticklabels(list(df_cos['Country of Sponsor/Collaborator']), rotation='vertical', fontdict={'fontsize': 14})
ax.set_xlabel(' ')
...
######lurong plot
ax = df_d_clean.plot.barh(figsize=(20, 75), x='Treatment', y='Count', color='green')
ax.invert_yaxis()
for i in ax.patches:
...
ssh.connect('39.98.42.159', username='ghddirun', password='GHDDI@ali1')

with SCPClient(ssh.get_transport()) as scp:
    scp.put('druglist.csv', '/home/ghddirun/ghddi-oss/file/druglist.csv') #druglist.csv as sample for now
    #use next 3 for final update:
    #scp.put('figure_countries.png', '/home/ghddirun/ghddi-oss/file/figure_countries.png')
    #scp.put('figure_drugs_treatment.png', '/home/ghddirun/ghddi-oss/file/figure_drugs_treatment.png')
    #scp.put('figure_types.png', '/home/ghddirun/ghddi-oss/file/figure_types.png')

ssh.close()

'''
#TODO: on linux, cron command
vi /etc/crontab

1 5 * * * python /$FILEPATH/drugs_biologics_vaccines.py
